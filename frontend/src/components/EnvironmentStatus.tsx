  ...
  return (
    <div className="grid gap-5 md:grid-cols-3">
      {envs.map((env) => (
        <div
          key={env.name}
          className="group p-6 rounded-[var(--radius-xl)] bg-[var(--color-surface)] border border-[var(--color-border)] hover:border-[var(--color-border)]/80 transition-all shadow-[var(--shadow-sm)]"
        >
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="font-semibold text-[var(--color-text)] capitalize block">{env.name}</span>
                <span className="text-xs text-[var(--color-text-muted)] font-medium uppercase tracking-wider">{env.type}</span>
              </div>
            </div>
            <span className={`px-2.5 py-1 rounded-[var(--radius-sm)] text-xs font-medium ${
              env.status === 'ok'
                ? 'bg-[var(--color-success-muted)] text-[var(--color-success)]'
                : 'bg-[var(--color-warning-muted)] text-[var(--color-warning)]'
            }`}>
              {env.status === 'ok' ? 'In sync' : 'Drift detected'}
            </span>
          </div>
          <p className="text-sm text-[var(--color-text-secondary)] mb-4">Last report: {env.lastReport}</p>
          <div className="pt-4 border-t border-[var(--color-border)]">
            <code className="text-xs text-[var(--color-text-muted)] font-mono bg-[var(--color-bg)] px-2 py-1.5 rounded-[var(--radius-sm)] block overflow-x-auto">
